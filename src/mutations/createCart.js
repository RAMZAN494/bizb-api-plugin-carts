...
export default async function createCart(context, input) {
  ...
  // Check if items are provided or if the cart should be created without items
  if (!shouldCreateWithoutItems && (!Array.isArray(items) || !items.length)) {
    throw new ReactionError(
      "invalid-param",
      "A cart may not be created without at least one item in it"
    );
  }
  ...
  if (accountId) {
    ...
    if (existingCart) {
      throw new ReactionError(
        "cart-found",
        "Each account may have only one cart per shop at a time"
      );
    }
  }
  ...
  const newCart = {
    ...
    anonymousAccessToken:
      anonymousAccessToken && hashToken(anonymousAccessToken),
    currencyCode: cartCurrencyCode,
    createdAt,
    items: updatedItemList, // Empty if no items were added
    shopId,
    updatedAt: createdAt,
    workflow: {
      status: "new",
  ...
  if (!createReferenceIdFunctions || createReferenceIdFunctions.length === 0) {
    ...
    if (typeof referenceId !== "string") {
      throw new ReactionError(
        "invalid-parameter",
        "createCartReferenceId returned a non-string value"
      );
    }
    if (createReferenceIdFunctions.length > 1) {
      Logger.warn(
        "More than one createCartReferenceId function defined. Using first one defined"
      );
    }
  }
...
//  * @param {Object} context - an object containing the per-request state
//  * @param {Object} input - mutation input
//  * @param {String} input.items - An array of cart items to add to the new cart. Can be empty if `allowEmptyCart` is set to true.
//  * @param {String} input.shopId - The ID of the shop that will own this cart
//  * @param {Boolean} [input.shouldCreateWithoutItems] - Create even if `items` is empty or becomes empty
//  *   due to price mismatches? Default is false. This is for backwards compatibility with old Meteor code
//  *   that creates the cart prior to adding items and should not be set to `true` in new code.
//  * @param {Boolean} [input.allowEmptyCart] - Allows the creation of an empty cart if set to true. Default is false.
//  * @returns {Promise<Object>} An object with `cart`, `minOrderQuantityFailures`, and `incorrectPriceFailures` properties.
//  *   `cart` will be null if all prices were incorrect. If at least one item could be added,
//  *   then the cart will have been created and returned, but `incorrectPriceFailures` and
//  *   `minOrderQuantityFailures` may still contain other failures that the caller should
...
//     items,
//     shopId,
//     shouldCreateWithoutItems = false,
//     allowEmptyCart = false,
//   } = input;
//   const { collections, accountId = null, getFunctionsOfType } = context;
//   const { Cart, Shops } = collections;

//   // Check for empty items, but allow empty cart if allowEmptyCart is set to true
//   if (
//     !allowEmptyCart &&
...
//   // If all input items were invalid and allowEmptyCart is not set, don't create a cart
//   if (
...
//     currencyCode: cartCurrencyCode,
//     createdAt,
//     items: updatedItemList, // This can be empty now
//     shopId,
//     updatedAt: createdAt,
//     workflow: {
//       status: "new",
